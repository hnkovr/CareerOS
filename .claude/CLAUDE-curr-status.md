...
- infra (2026-08-26 evening, **resolved**): the Docker daemon died twice and another lane briefly ran a Homebrew Postgres 17 on 5432 (`/tmp/careeros-pg17`) with only `careeros` (empty) + `careeros_test_d2`, which made unrelated code look broken (`database "careeros" does not exist`, silently skipped db tests). The container (pgvector/pg16) holds the port again with every lane database intact; the /tmp cluster is stopped and disposable. Lesson: identify the server (`lsof -nP -iTCP:5432 -sTCP:LISTEN`) before debugging code
