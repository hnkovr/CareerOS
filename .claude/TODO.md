...
- [x] P3 assistants via tool-calling (§55, [ADR 014](../docs/adr/014-assistants-tool-calling.md)): `chat_with_tools` in the `AIProvider` port (Anthropic, OpenAI-compatible, Fake), gateway-owned loop `AIService.with_tools` with one ledger row + tool trace, `modules/assistant` with 6 read-only tools over service layers, answer guard (unknown ids / unseen numbers → withheld); `POST /api/assistant/ask`, `careeros assistant ask`, `/assistant` page
- [ ] assistant follow-ups: Suggestion-producing write tools (draft_reply, create_application) behind approval; streaming turns for web/bot; per-tool token budgets; `search_facts` over the unified index when the vault outgrows keyword matching
- [x] §53 workflows with WAIT_FOR_APPROVAL ([ADR 017](../docs/adr/017-workflows-wait-for-approval.md)): `modules/workflows` step engine (`workflow_run` table, migration c4d8e2f1a9b3), approval steps → Suggestion → resume/cancel; workflows `apply` (analyze → select_cv → generate_cv → ⏸ draft_message → create_application at preparing/proposal) and `follow_up` (review → ⏸ draft_follow_up (AI draft number-guarded) → record + schedule +5d); API/CLI/`/workflows` page + buttons on opportunity and application pages
- [x] `start_workflow` assistant tool (ADR 014 ↔ 017): the assistant can start an apply/follow_up run that stops at its gate; `read_only=False` surfaced in `/assistant/tools`; test proves nothing enters the pipeline
- [x] daily follow-up sweep: `sweep_follow_ups` (one gated `follow_up` run per due/overdue follow-up, active runs dedupe) — `POST /workflows/sweep`, `careeros workflows sweep`, worker cron 08:00 UTC (`workflows.sweep_follow_ups`), web button
- [ ] workflow follow-ups: ARQ-backed long steps; a `send` gate once Gmail exists; bot buttons for pending gates
